...
export const ThemeButton = ({ screen }) => {
    ...
    return (
        <div className="relative z-50">
            ...
            >
                {(theme.theme === "dark" || (theme.theme === "system" && theme.systemTheme === "dark")) && <MoonIcon className={twclsx("w-5 text-secondary-400", "pointer-events-none")} />}
                {(theme.theme === "light" || (theme.theme === "system" && theme.systemTheme === "light")) && <SunIcon className={twclsx("w-5 text-primary-700", "pointer-events-none")} />}
            </UnstyledButton>

            <AnimatePresence mode="wait">{theme.dropdownIsOpen && <ThemeMenu changeTheme={theme.changeTheme} onClose={theme.closeDropdown} theme={theme.theme} />}</AnimatePresence>
        </div>
    );
};
