...
export default function PrevNext({prev, next, allPages}){
	...
	return (
			<div className="fixed bottom-0 left-0 right-0 z-[99] m-4  ">
				<div
						className="mx-auto flex w-48 justify-center gap-4 rounded-xl border border-white/40 bg-black/50 px-4 py-2 backdrop-blur-sm backdrop-filter">
					<div className="flex gap-2 cursor-pointer bg-transparent">
						<button
								onClick={handlePrev}
								disabled={!prev}
									...
									<DropdownMenuItem>
										<div className="grid grid-cols-1 md:grid-cols-2 gap-4 cursor-pointer">
											{allPages.map((page, index) => (
													<Link key={index} href={`/present/page/${page.slug}`} className="capitalize">
