...
export default async function Page() {
    ...
    return (
        <main className="container relative mx-auto scroll-my-12 overflow-auto p-4 md:p-16 print:p-12 print:my-12">
            <section className="mx-auto w-full max-w-2xl space-y-8 bg-background print:space-y-6">
                <div className="flex items-center justify-between">
                    <div className="flex-1 space-y-1.5">
                        <h1 className="text-2xl font-bold">{RESUME_DATA.name}</h1>

                        <p className="max-w-md text-pretty font-mono text-sm text-primary-400">{RESUME_DATA.about}</p>
                        ...
                        <div className="flex gap-x-1 pt-1 font-mono text-sm text-primary-400 print:hidden">
                            ...
                            {RESUME_DATA.contact.social.map((social) => (
                                <Button key={social.name} className="size-8 text-white" size="icon" asChild>
                                    <a href={social.url}>
                ...
                <Section>
                    ...
                    <p className="text-pretty font-mono text-sm text-primary-400">{RESUME_DATA.summary}</p>
                </Section>
                <Section>
                    ...
                    {RESUME_DATA.work.map((work) => {
                        return (
                            <div key={work.company}>
                                ...
                                <div className="mt-2 text-sm text-primary-400">{work.description}</div>
                            </div>
                        );
                    })}
                </Section>
                    ...
                    {RESUME_DATA.education.map((education) => {
                        return (
                            <div key={education.school}>
                                <div>
                                    <div className="flex items-center justify-between gap-x-2 text-base">
                                        <h5 className="font-semibold leading-none">{education.school}</h5>
                                        <div className="text-sm tabular-nums text-primary-400">
                                            {education.start} - {education.end}
                                        </div>
                                    </div>
                                </div>
                                <div className="mt-2 text-sm text-primary-400">{education.degree}</div>
                            </div>
                        );
                    })}
                </Section>
                <Section className="print-force-new-page print:pt-12">
                    ...
                    <div className="flex flex-wrap gap-1">
                        {tools.flatMap((tool) => tool.item.map((item, idx) => <IconText key={idx} className="group/tools" icon={<DynamicIcon fill="#fffa" size={16} name={item} />} text={item} />))}
                    </div>
                </Section>
                        ...
                        {projects.map(({ link, title, description, stacks }) => {
                            return (
                                <div className="flex flex-col overflow-hidden rounded-xl border border-primary-600 p-3">
                                    <div className="flex flex-col gap-2">
                                        {link && <img src={getFavicon(link)} className="w-4 h-4 inline-block" />}
                                        <div className="space-y-1">
                                            ...
                                            <div className="hidden font-mono text-xs underline print:flex">{link?.replace("https://", "").replace("www.", "").replace("/", "")}</div>
                                            <div className="font-mono text-xs text-primary-400">{description}</div>
                                        </div>
                                    </div>
                                    <div className="mt-auto flex">
                                        <div className="mt-2 flex flex-wrap gap-1">
                                            {stacks.map((stack) => (
                                                <Badge className="px-1 py-0 text-[10px]" variant="secondary" key={stack}>
                                                    {stack}
                                                </Badge>
                                            ))}
                                        </div>
